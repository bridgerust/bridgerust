...
use serde_json::Value; // Make sure generic Value is available if needed, though we should map to JS object.
...
impl Collection {
    ...
    pub async fn insert(&self, points: Vec<Point>) -> Result<()> {
        ...
        let rust_points: Vec<RustPoint> = points.into_iter().map(|p| {
            RustPoint {
                id: p.id,
                // Cast f64 to f32
    ...
    pub async fn search(
        ...
        let builder = inner.search(vec_f32).await
            .limit(limit)
    ...
    pub async fn insert_batch(&self, points: Vec<Point>, batch_size: Option<u32>) -> Result<()> {
        ...
        inner.insert_batch(rust_points, size).await
            .map_err(|e| Error::from_reason(e.to_string()))
    }
}
