...
def test_only_lowercase_exports_are_available():
    assert hasattr(bridgetime, "bridge_time")
    assert hasattr(bridgetime, "bridge_duration")
    assert not hasattr(bridgetime, "BridgeTime")
    assert not hasattr(bridgetime, "BridgeDuration")


def test_parse_add_and_format():
    dt = bridge_time.parse("2026-02-22T10:15:30Z", "UTC")
...
def test_parse_format_supports_custom_patterns():
    dt = bridge_time.parse_format("22/02/2026 10:15", "DD/MM/YYYY HH:mm", "UTC")
    assert dt.format("YYYY-MM-DD HH:mm:ss") == "2026-02-22 10:15:00"

    date_only = bridge_time.parse_format("2026/02/22", "YYYY/MM/DD", "UTC")
    assert date_only.format("YYYY-MM-DD HH:mm:ss") == "2026-02-22 00:00:00"


def test_array_min_max_clamp_and_dst_helpers():
    dt = bridge_time.from_array([2026, 1, 22, 10, 15, 30, 250], "UTC")
    assert dt.to_array() == [2026, 1, 22, 10, 15, 30, 250]

    overflow = bridge_time.from_array([2026, 12, 1], "UTC")
    assert overflow.format("YYYY-MM-DD") == "2027-01-01"

    a = bridge_time.parse("2026-02-22T10:00:00Z", "UTC")
    b = bridge_time.parse("2026-02-22T11:00:00Z", "UTC")
    c = bridge_time.parse("2026-02-22T12:00:00Z", "UTC")
    assert bridge_time.min(a, b).unix_ms() == a.unix_ms()
    assert bridge_time.max(a, b).unix_ms() == b.unix_ms()
    assert a.clamp(b, c).unix_ms() == b.unix_ms()
    assert c.clamp(a, b).unix_ms() == b.unix_ms()
    assert b.clamp(a, c).unix_ms() == b.unix_ms()

    ny_winter = bridge_time.parse("2026-01-15T12:00:00Z", "America/New_York")
    ny_summer = bridge_time.parse("2026-07-15T12:00:00Z", "America/New_York")
    assert ny_winter.is_dst() is False
    assert ny_summer.is_dst() is True


def test_timezone_conversion_preserves_instant():
    dt = bridge_time.parse("2026-02-22T12:00:00Z", "UTC")
...
def test_get_set_and_calendar_helpers():
    dt = bridge_time.parse("2026-02-22T10:15:30.250Z", "UTC")
...
def test_unit_comparisons_and_between():
    morning = bridge_time.parse("2026-02-22T10:15:30Z", "UTC")
    evening = bridge_time.parse("2026-02-22T23:59:00Z", "UTC")
    next_day = bridge_time.parse("2026-02-23T00:00:00Z", "UTC")
...
def test_relative_time_helpers():
    base = bridge_time.parse("2026-02-22T10:00:00Z", "UTC")
    future = bridge_time.parse("2026-02-22T10:30:00Z", "UTC")
    past = bridge_time.parse("2026-02-22T09:30:00Z", "UTC")
...
def test_duration_helpers():
    ...
    thirty = bridge_duration.from_minutes(30)
    ...
    static_duration = bridge_time.duration(2, "hour")
