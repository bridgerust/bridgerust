...
# Test configuration
TEST_DIMENSION = 128
TEST_COLLECTION = "kabod_integration_test"
...
class TestQdrantAdapter:
    ...
    def client(self):
        ...
        return KabodClient(provider="qdrant", url="http://localhost:6334")
...
class TestChromaAdapter:
    ...
    def client(self):
        from kabod import KabodClient
        return KabodClient(provider="chroma", url="http://localhost:8000")
...
class TestWeaviateAdapter:
    ...
    def client(self):
        from kabod import KabodClient
        return KabodClient(provider="weaviate", url="http://localhost:8080")
    
    @pytest.fixture
    def collection(self, client):
        return client.collection("KabodIntegrationTest")
...
class TestMilvusAdapter:
    ...
    async def test_crud_operations(self):
        ...
        client = await KabodClient.new_async(provider="milvus", url="http://localhost:19530")
...
class TestPgvectorAdapter:
    ...
    async def test_crud_operations(self):
        ...
        client = await KabodClient.new_async(
            provider="pgvector",
            url="postgresql://kabod:kabod_test@localhost:5432/kabod_test"
        )
        collection = client.collection(TEST_COLLECTION)
...
class TestLanceDBAdapter:
    ...
    async def test_crud_operations(self, tmp_path):
        ...
        client = await KabodClient.new_async(provider="lancedb", url=db_path)
        collection = client.collection(TEST_COLLECTION)
