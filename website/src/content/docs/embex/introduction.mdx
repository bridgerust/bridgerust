...
<Tabs>
  <TabItem label="Python">
    ```python 
    # Initialize properly for ANY provider (local or cloud) client =
    await EmbexClient.new_async("lancedb://./data") 
    ```

  </TabItem>
  <TabItem label="Node.js">
    ```typescript 
    // Initialize properly for ANY provider (local or cloud) const
    client = await EmbexClient.newAsync("lancedb://./data"); 
    ```
