...
Releases are automated via GitHub Actions workflows and triggered by git tags.
Use `vX.Y.Z` tags as the canonical release trigger (for example `v0.1.18`).
...
1. `.github/workflows/publish-crates-io.yml`: Publishes Rust crates (`v*` and legacy `bridgerust-v*` tags).
2. `.github/workflows/release-python.yml`: Builds and publishes Python wheels to PyPI.
3. `.github/workflows/release-node.yml`: Builds and publishes Node.js bindings and platform packages to npm.
4. `.github/workflows/release.yml`: Creates the GitHub Release page with autogenerated notes.
