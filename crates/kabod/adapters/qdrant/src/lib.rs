...
fn convert_key_condition(
    ...
    match condition {
        ...
        | KabodCondition::Lte(value) => {
            ...
            match condition {
                KabodCondition::Gt(_) => range.gt = Some(val),
                KabodCondition::Gte(_) => range.gte = Some(val),
                KabodCondition::Lt(_) => range.lt = Some(val),
                KabodCondition::Lte(_) => range.lte = Some(val),
                _ => unreachable!(),
            }
            Condition {
                condition_one_of: Some(ConditionOneOf::Field(FieldCondition {
