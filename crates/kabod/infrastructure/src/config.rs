...
impl KabodConfig {
    pub fn new() -> Result<Self> {
        let s = Config::builder()
            ...
            .build()
            .map_err(|e| KabodError::Config(format!("Failed to load config: {}", e)))?;

        s.try_deserialize()
            .map_err(|e| KabodError::Config(format!("Failed to deserialize config: {}", e)))
    }
