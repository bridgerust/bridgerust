    ...
    F: FnMut() -> std::pin::Pin<Box<dyn std::future::Future<Output = Result<T>> + Send>>,
{
    ...
    for attempt in 0..=config.max_retries {
        match f().await {
            ...
            Err(e) => {
                ...
                last_error = Some(KabodError::Other(anyhow::anyhow!("{}", error_msg)));
    ...
    Err(last_error.unwrap_or_else(|| {
        KabodError::Other(anyhow::anyhow!("Retry exhausted without error"))
    }))
}
        ...
        let result = retry_with_backoff(&config, {
            ...
            move || {
                ...
                Box::pin(async move {
                    ...
                    if attempts < 2 {
                        Err(KabodError::Connection("test".to_string()))
                    } else {
                        Ok::<(), KabodError>(())
                    }
                })
    ...
    async fn test_retry_fails_on_non_retryable_error() {
        ...
        let result: Result<()> = retry_with_backoff(&config, {
            ...
            move || {
                ...
                Box::pin(async move {
                    count.fetch_add(1, Ordering::SeqCst);
                    Err(KabodError::Validation("non-retryable".to_string()))
                })
            }
        })
        .await;
    ...
    async fn test_retry_exhausts_after_max_retries() {
        ...
        let result: Result<()> = retry_with_backoff(&config, {
            ...
            move || {
                ...
                Box::pin(async move {
                    count.fetch_add(1, Ordering::SeqCst);
                    Err(KabodError::Connection("always fails".to_string()))
                })
            }
        })
        .await;
