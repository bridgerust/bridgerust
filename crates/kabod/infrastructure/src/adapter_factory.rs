...
#[cfg(feature = "qdrant")]
use bridge_kabod_qdrant::QdrantAdapter;
#[cfg(feature = "pinecone")]
use bridge_kabod_pinecone::PineconeAdapter;
#[cfg(feature = "chroma")]
use bridge_kabod_chroma::ChromaAdapter;
#[cfg(feature = "lancedb")]
use bridge_kabod_lancedb::LanceDBAdapter;
#[cfg(feature = "pgvector")]
use bridge_kabod_pgvector::PgVectorAdapter;
...
impl AdapterFactory {
    ...
    pub fn create(config: &KabodConfig) -> Result<Arc<dyn VectorDatabase>> {
        #[cfg(feature = "qdrant")]
        ...
        if config.provider == "pinecone" {
            let api_key = config.api_key.as_ref().ok_or_else(|| {
                bridge_kabod_core::error::KabodError::Config(
        ...
        Err(bridge_kabod_core::error::KabodError::Config(format!(
    ...
    pub async fn create_async(config: &KabodConfig) -> Result<Arc<dyn VectorDatabase>> {
        #[cfg(feature = "lancedb")]
        if config.provider == "lancedb" {
