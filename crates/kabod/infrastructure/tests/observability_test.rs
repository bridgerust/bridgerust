use bridge_kabod_infrastructure::observability::{KabodMetrics, MetricsSnapshot, Timer};
...
fn test_metrics_concurrent_recording() {
    ...
    let metrics = Arc::new(KabodMetrics::new());
    let mut handles = vec![];
...
fn test_metrics_snapshot_isolation() {
    let metrics1 = KabodMetrics::new();
    let metrics2 = KabodMetrics::new();
