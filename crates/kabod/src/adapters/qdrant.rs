...
use qdrant_client::qdrant::{
    vectors_config::Config, CreateCollectionBuilder, Distance, PointStruct, SearchPointsBuilder, VectorParams,
    VectorsConfig, Filter, ScoredPoint, UpsertPointsBuilder, DeletePointsBuilder
};
use qdrant_client::Payload;

use crate::db::VectorDatabase;
use crate::error::{Result, KabodError};
use crate::types::{self, CollectionSchema, DistanceMetric, MetadataUpdate, Point, VectorQuery, SearchResult};
...
impl VectorDatabase for QdrantAdapter {
    async fn create_collection(&self, schema: &CollectionSchema) -> Result<()> {
        ...
        let details = CreateCollectionBuilder::new(schema.name.clone())
    ...
    async fn search(&self, query: &VectorQuery) -> Result<Vec<SearchResult>> {
        let mut builder = SearchPointsBuilder::new(query.collection.clone(), query.vector.clone(), query.top_k as u64)
        ...
        Ok(result.result.into_iter().map(convert_scored_point).collect())
    }
...
fn convert_scored_point(point: ScoredPoint) -> SearchResult {
    let id = point.id.and_then(|id| id.point_id_options).map(|opt| match opt {
    ...
    SearchResult {
        ...
        score: point.score,
        vector,
        metadata: Some(point.payload.into_iter().map(|(k, v)| (k, v.into())).collect()),
    }
}
