...
use bridge_kabod::migration::MigrationManager;
...
struct MockDatabase {
    ...
    pub points: Arc<Mutex<Vec<(String, Vec<Point>)>>>,
}
...
impl VectorDatabase for MockDatabase {
    async fn create_collection(&self, schema: &CollectionSchema) -> Result<()> {
        ...
        if collections.contains(&schema.name) {
            return Err(bridge_kabod::error::KabodError::CollectionExists(schema.name.clone()));
        }
    ...
    async fn search(&self, _query: &bridge_kabod_core::types::VectorQuery) -> Result<bridge_kabod_core::types::SearchResponse> {
        ...
        for (coll_name, points) in stored.iter() {
            if coll_name == collection {
                for point in points {
                    results.push(bridge_kabod_core::types::SearchResult {
                        id: point.id.clone(),
                        score: 1.0,
        ...
        Ok(bridge_kabod_core::types::SearchResponse {
            results,
    ...
    async fn update_metadata(&self, _collection: &str, _updates: Vec<bridge_kabod_core::types::MetadataUpdate>) -> Result<()> {
        Ok(())
    }
}
...
impl Migration for TestMigration {
    ...
    async fn up(&self, db: Arc<dyn VectorDatabase>) -> Result<()> {
        if self.should_fail {
            return Err(bridge_kabod::error::KabodError::Validation("Migration failed".to_string()));
        }
    ...
    async fn down(&self, db: Arc<dyn VectorDatabase>) -> Result<()> {
        if self.rollback_fails {
            return Err(bridge_kabod::error::KabodError::Validation("Rollback failed".to_string()));
        }
        
        db.delete_collection(&format!("test_collection_{}", self.version)).await
    }
}
...
async fn test_rollback_migrations() {
    ...
    manager.rollback_last(2, &migrations_for_rollback).await.unwrap();
...
async fn test_rollback_specific_migrations() {
    ...
    manager.rollback_migrations(&to_rollback, &migrations_for_rollback).await.unwrap();
